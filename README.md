...
int main()
{
    ...
    printf("Gaja's private key (x): ");
    scanf("%lld", &bob_private_key);

    long long bob_public_key = modexp(g, bob_private_key, p);
    printf("lakshmi's public key (y): %lld\n", bob_public_key);

    printf("Gaja, enter the message (M) you want to send: ");
    scanf("%lld", &message);

    printf("Lakshmi's private key (k): ");
    scanf("%lld", &k);
    ...
    printf("Gaja sends encrypted message (c1, c2): (%lld, %lld)\n", c1, c2);
    ...
    printf("lakshmi decrypts the message: %lld\n", decrypted_message);

    return 0;
}
```
## Output:
<img width="1621" height="903" alt="image" src="https://github.com/user-attachments/assets/f0b3fa6c-d13e-4dc2-9c6c-190391ec9df8" />
